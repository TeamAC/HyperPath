...
public class AdvertisersJpaController implements Serializable {
    ...
    public AdvertisersJpaController(UserTransaction utx, EntityManagerFactory emf) {
        this.utx = utx;
        this.emf = emf;
    }
    ...
    public void create(Advertisers advertisers) throws PreexistingEntityException, RollbackFailureException, Exception {
        ...
        try {
            ...
            for (Ads adsListAds : advertisers.getAdsList()) {
                Advertisers oldAdvertisersOfAdsListAds = adsListAds.getAdvertisers();
                ...
                if (oldAdvertisersOfAdsListAds != null) {
                    ...
                    oldAdvertisersOfAdsListAds = em.merge(oldAdvertisersOfAdsListAds);
                }
            }
            utx.commit();
        } catch (Exception ex) {
            ...
            if (findAdvertisers(advertisers.getAdvertisersPK()) != null) {
                throw new PreexistingEntityException("Advertisers " + advertisers + " already exists.", ex);
            }
            throw ex;
        } finally {
            if (em != null) {
    ...
    public void edit(Advertisers advertisers) throws IllegalOrphanException, NonexistentEntityException, RollbackFailureException, Exception {
        ...
        try {
            utx.begin();
            em = getEntityManager();
            Advertisers persistentAdvertisers = em.find(Advertisers.class, advertisers.getAdvertisersPK());
            ...
            for (Ads adsListOldAds : adsListOld) {
                if (!adsListNew.contains(adsListOldAds)) {
                    ...
                    illegalOrphanMessages.add("You must retain Ads " + adsListOldAds + " since its advertisers field is not nullable.");
                }
            }
            if (illegalOrphanMessages != null) {
            ...
            for (Ads adsListNewAds : adsListNew) {
                if (!adsListOld.contains(adsListNewAds)) {
                    Advertisers oldAdvertisersOfAdsListNewAds = adsListNewAds.getAdvertisers();
                    adsListNewAds.setAdvertisers(advertisers);
                    adsListNewAds = em.merge(adsListNewAds);
                    if (oldAdvertisersOfAdsListNewAds != null && !oldAdvertisersOfAdsListNewAds.equals(advertisers)) {
                        oldAdvertisersOfAdsListNewAds.getAdsList().remove(adsListNewAds);
                        oldAdvertisersOfAdsListNewAds = em.merge(oldAdvertisersOfAdsListNewAds);
                    }
                }
            }
            utx.commit();
    ...
    public void destroy(Integer id) throws IllegalOrphanException, NonexistentEntityException, RollbackFailureException, Exception {
        ...
        try {
            ...
            for (Ads adsListOrphanCheckAds : adsListOrphanCheck) {
                ...
                illegalOrphanMessages.add("This Advertisers (" + advertisers + ") cannot be destroyed since the Ads " + adsListOrphanCheckAds + " in its adsList field has a non-nullable advertisers field.");
            }
            if (illegalOrphanMessages != null) {
                throw new IllegalOrphanException(illegalOrphanMessages);
            }
