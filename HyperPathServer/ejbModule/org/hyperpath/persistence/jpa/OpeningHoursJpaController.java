    ...
    public void create(OpeningHours openingHours) throws RollbackFailureException, Exception {
        ...
        try {
            ...
            for (Services servicesListServices : openingHours.getServicesList()) {
                OpeningHours oldOpeningHoursOfServicesListServices = servicesListServices.getOpeningHours();
                ...
                if (oldOpeningHoursOfServicesListServices != null) {
                    oldOpeningHoursOfServicesListServices.getServicesList().remove(servicesListServices);
                    oldOpeningHoursOfServicesListServices = em.merge(oldOpeningHoursOfServicesListServices);
                }
            }
            utx.commit();
        } catch (Exception ex) {
        ...
        try {
            ...
            for (Services servicesListOldServices : servicesListOld) {
                if (!servicesListNew.contains(servicesListOldServices)) {
                    ...
                    illegalOrphanMessages.add("You must retain Services " + servicesListOldServices + " since its openingHours field is not nullable.");
                }
            }
            if (illegalOrphanMessages != null) {
            ...
            for (Services servicesListNewServices : servicesListNew) {
                if (!servicesListOld.contains(servicesListNewServices)) {
                    OpeningHours oldOpeningHoursOfServicesListNewServices = servicesListNewServices.getOpeningHours();
                    servicesListNewServices.setOpeningHours(openingHours);
                    servicesListNewServices = em.merge(servicesListNewServices);
                    if (oldOpeningHoursOfServicesListNewServices != null && !oldOpeningHoursOfServicesListNewServices.equals(openingHours)) {
                        oldOpeningHoursOfServicesListNewServices.getServicesList().remove(servicesListNewServices);
                        oldOpeningHoursOfServicesListNewServices = em.merge(oldOpeningHoursOfServicesListNewServices);
                    }
                }
            }
            utx.commit();
    ...
    public void destroy(Integer id) throws IllegalOrphanException, NonexistentEntityException, RollbackFailureException, Exception {
        ...
        try {
            ...
            for (Services servicesListOrphanCheckServices : servicesListOrphanCheck) {
                ...
                illegalOrphanMessages.add("This OpeningHours (" + openingHours + ") cannot be destroyed since the Services " + servicesListOrphanCheckServices + " in its servicesList field has a non-nullable openingHours field.");
            }
            if (illegalOrphanMessages != null) {
                throw new IllegalOrphanException(illegalOrphanMessages);
            }
