...
public class EntitiesJpaController implements Serializable {
  ...
  public void create(Entities entities) throws RollbackFailureException,
      Exception {
    ...
    try {
      ...
      for (Emails emailsListEmailsToAttach : entities.getEmailsList()) {
        emailsListEmailsToAttach = em.getReference(
            emailsListEmailsToAttach.getClass(),
            emailsListEmailsToAttach.getId());
      ...
      for (Faxes faxesListFaxesToAttach : entities.getFaxesList()) {
        faxesListFaxesToAttach = em.getReference(
            faxesListFaxesToAttach.getClass(),
            faxesListFaxesToAttach.getId());
      ...
      for (Phones phonesListPhonesToAttach : entities.getPhonesList()) {
        phonesListPhonesToAttach = em.getReference(
            phonesListPhonesToAttach.getClass(),
            phonesListPhonesToAttach.getId());
      ...
      for (Address addressListAddressToAttach : entities.getAddressList()) {
        addressListAddressToAttach = em.getReference(
            addressListAddressToAttach.getClass(),
            addressListAddressToAttach.getId());
      ...
      for (Advertisers advertisersListAdvertisersToAttach : entities
          .getAdvertisersList()) {
        advertisersListAdvertisersToAttach = em.getReference(
            advertisersListAdvertisersToAttach.getClass(),
            advertisersListAdvertisersToAttach.getAdvertisersPK());
      ...
      for (Services servicesListServicesToAttach : entities
          .getServicesList()) {
      ...
      for (Advertisers advertisersListAdvertisers : entities
          .getAdvertisersList()) {
        Entities oldEntitiesOfAdvertisersListAdvertisers = advertisersListAdvertisers
            .getEntities();
        advertisersListAdvertisers.setEntities(entities);
        advertisersListAdvertisers = em
            .merge(advertisersListAdvertisers);
        if (oldEntitiesOfAdvertisersListAdvertisers != null) {
          oldEntitiesOfAdvertisersListAdvertisers
              .getAdvertisersList().remove(
                  advertisersListAdvertisers);
          oldEntitiesOfAdvertisersListAdvertisers = em
              .merge(oldEntitiesOfAdvertisersListAdvertisers);
        }
      }
      for (Services servicesListServices : entities.getServicesList()) {
        Entities oldEntitiesOfServicesListServices = servicesListServices
            .getEntities();
        ...
        if (oldEntitiesOfServicesListServices != null) {
          oldEntitiesOfServicesListServices.getServicesList().remove(
              servicesListServices);
          oldEntitiesOfServicesListServices = em
              .merge(oldEntitiesOfServicesListServices);
        }
      }
      for (Clients clientsListClients : entities.getClientsList()) {
        Entities oldEntitiesOfClientsListClients = clientsListClients
            .getEntities();
        ...
        if (oldEntitiesOfClientsListClients != null) {
          oldEntitiesOfClientsListClients.getClientsList().remove(
              clientsListClients);
          oldEntitiesOfClientsListClients = em
              .merge(oldEntitiesOfClientsListClients);
        }
      }
      utx.commit();
    } catch (Exception ex) {
  ...
  public void edit(Entities entities) throws IllegalOrphanException,
      NonexistentEntityException, RollbackFailureException, Exception {
    EntityManager em = null;
    try {
      utx.begin();
      em = getEntityManager();
      Entities persistentEntities = em.find(Entities.class,
          entities.getId());
      ...
      List<Advertisers> advertisersListOld = persistentEntities
          .getAdvertisersList();
      List<Advertisers> advertisersListNew = entities
          .getAdvertisersList();
      List<Services> servicesListOld = persistentEntities
          .getServicesList();
      ...
      for (Advertisers advertisersListOldAdvertisers : advertisersListOld) {
        if (!advertisersListNew.contains(advertisersListOldAdvertisers)) {
          ...
          illegalOrphanMessages.add("You must retain Advertisers "
              + advertisersListOldAdvertisers
              + " since its entities field is not nullable.");
        }
      }
      ...
      for (Clients clientsListOldClients : clientsListOld) {
        if (!clientsListNew.contains(clientsListOldClients)) {
          ...
          illegalOrphanMessages.add("You must retain Clients "
              + clientsListOldClients
              + " since its entities field is not nullable.");
        }
      }
      if (illegalOrphanMessages != null) {
      ...
      for (Emails emailsListNewEmailsToAttach : emailsListNew) {
        emailsListNewEmailsToAttach = em.getReference(
            emailsListNewEmailsToAttach.getClass(),
            emailsListNewEmailsToAttach.getId());
      ...
      for (Faxes faxesListNewFaxesToAttach : faxesListNew) {
        faxesListNewFaxesToAttach = em.getReference(
            faxesListNewFaxesToAttach.getClass(),
            faxesListNewFaxesToAttach.getId());
      ...
      for (Phones phonesListNewPhonesToAttach : phonesListNew) {
        phonesListNewPhonesToAttach = em.getReference(
            phonesListNewPhonesToAttach.getClass(),
            phonesListNewPhonesToAttach.getId());
      ...
      for (Address addressListNewAddressToAttach : addressListNew) {
        addressListNewAddressToAttach = em.getReference(
            addressListNewAddressToAttach.getClass(),
            addressListNewAddressToAttach.getId());
      ...
      for (Advertisers advertisersListNewAdvertisersToAttach : advertisersListNew) {
        advertisersListNewAdvertisersToAttach = em.getReference(
            advertisersListNewAdvertisersToAttach.getClass(),
            advertisersListNewAdvertisersToAttach
                .getAdvertisersPK());
        attachedAdvertisersListNew
            .add(advertisersListNewAdvertisersToAttach);
      }
      ...
      for (Advertisers advertisersListNewAdvertisers : advertisersListNew) {
        if (!advertisersListOld.contains(advertisersListNewAdvertisers)) {
          Entities oldEntitiesOfAdvertisersListNewAdvertisers = advertisersListNewAdvertisers
              .getEntities();
          advertisersListNewAdvertisers.setEntities(entities);
          advertisersListNewAdvertisers = em
              .merge(advertisersListNewAdvertisers);
          if (oldEntitiesOfAdvertisersListNewAdvertisers != null
              && !oldEntitiesOfAdvertisersListNewAdvertisers
                  .equals(entities)) {
            oldEntitiesOfAdvertisersListNewAdvertisers
                .getAdvertisersList().remove(
                    advertisersListNewAdvertisers);
            oldEntitiesOfAdvertisersListNewAdvertisers = em
                .merge(oldEntitiesOfAdvertisersListNewAdvertisers);
          }
        }
      }
      for (Services servicesListNewServices : servicesListNew) {
        if (!servicesListOld.contains(servicesListNewServices)) {
          Entities oldEntitiesOfServicesListNewServices = servicesListNewServices
              .getEntities();
          servicesListNewServices.setEntities(entities);
          servicesListNewServices = em.merge(servicesListNewServices);
          if (oldEntitiesOfServicesListNewServices != null
              && !oldEntitiesOfServicesListNewServices
                  .equals(entities)) {
            oldEntitiesOfServicesListNewServices.getServicesList()
                .remove(servicesListNewServices);
            oldEntitiesOfServicesListNewServices = em
                .merge(oldEntitiesOfServicesListNewServices);
          }
        }
      }
      for (Clients clientsListNewClients : clientsListNew) {
        if (!clientsListOld.contains(clientsListNewClients)) {
          Entities oldEntitiesOfClientsListNewClients = clientsListNewClients
              .getEntities();
          clientsListNewClients.setEntities(entities);
          clientsListNewClients = em.merge(clientsListNewClients);
          if (oldEntitiesOfClientsListNewClients != null
              && !oldEntitiesOfClientsListNewClients
                  .equals(entities)) {
            oldEntitiesOfClientsListNewClients.getClientsList()
                .remove(clientsListNewClients);
            oldEntitiesOfClientsListNewClients = em
                .merge(oldEntitiesOfClientsListNewClients);
          }
        }
      }
      utx.commit();
      ...
      if (msg == null || msg.length() == 0) {
        ...
        if (findEntities(id) == null) {
          throw new NonexistentEntityException(
              "The entities with id " + id + " no longer exists.");
        }
      }
      throw ex;
    } finally {
  ...
  public void destroy(Integer id) throws IllegalOrphanException,
    ...
    try {
      ...
      try {
        ...
        throw new NonexistentEntityException("The entities with id "
            + id + " no longer exists.", enfe);
      }
      List<String> illegalOrphanMessages = null;
      List<Advertisers> advertisersListOrphanCheck = entities
          .getAdvertisersList();
      for (Advertisers advertisersListOrphanCheckAdvertisers : advertisersListOrphanCheck) {
        ...
        illegalOrphanMessages
            .add("This Entities ("
                + entities
                + ") cannot be destroyed since the Advertisers "
                + advertisersListOrphanCheckAdvertisers
                + " in its advertisersList field has a non-nullable entities field.");
      }
      ...
      for (Services servicesListOrphanCheckServices : servicesListOrphanCheck) {
        ...
        illegalOrphanMessages
            .add("This Entities ("
                ...
                + " in its servicesList field has a non-nullable entities field.");
      }
      ...
      for (Clients clientsListOrphanCheckClients : clientsListOrphanCheck) {
        ...
        illegalOrphanMessages
            .add("This Entities ("
                + entities
                + ") cannot be destroyed since the Clients "
                + clientsListOrphanCheckClients
                + " in its clientsList field has a non-nullable entities field.");
      }
      if (illegalOrphanMessages != null) {
        throw new IllegalOrphanException(illegalOrphanMessages);
      }
  ...
  private List<Entities> findEntitiesEntities(boolean all, int maxResults,
                                              int firstResult) {
