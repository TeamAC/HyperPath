    ...
    public void create(Categories categories) throws RollbackFailureException, Exception {
        ...
        try {
            ...
            for (Services servicesListServices : categories.getServicesList()) {
                Categories oldCategoriesOfServicesListServices = servicesListServices.getCategories();
                ...
                if (oldCategoriesOfServicesListServices != null) {
                    oldCategoriesOfServicesListServices.getServicesList().remove(servicesListServices);
                    oldCategoriesOfServicesListServices = em.merge(oldCategoriesOfServicesListServices);
                }
            }
            utx.commit();
        } catch (Exception ex) {
        ...
        try {
            ...
            for (Services servicesListOldServices : servicesListOld) {
                if (!servicesListNew.contains(servicesListOldServices)) {
                    ...
                    illegalOrphanMessages.add("You must retain Services " + servicesListOldServices + " since its categories field is not nullable.");
                }
            }
            if (illegalOrphanMessages != null) {
            ...
            for (Services servicesListNewServices : servicesListNew) {
                if (!servicesListOld.contains(servicesListNewServices)) {
                    Categories oldCategoriesOfServicesListNewServices = servicesListNewServices.getCategories();
                    servicesListNewServices.setCategories(categories);
                    servicesListNewServices = em.merge(servicesListNewServices);
                    if (oldCategoriesOfServicesListNewServices != null && !oldCategoriesOfServicesListNewServices.equals(categories)) {
                        oldCategoriesOfServicesListNewServices.getServicesList().remove(servicesListNewServices);
                        oldCategoriesOfServicesListNewServices = em.merge(oldCategoriesOfServicesListNewServices);
                    }
                }
            }
            utx.commit();
    ...
    public void destroy(Integer id) throws IllegalOrphanException, NonexistentEntityException, RollbackFailureException, Exception {
        ...
        try {
            ...
            for (Services servicesListOrphanCheckServices : servicesListOrphanCheck) {
                ...
                illegalOrphanMessages.add("This Categories (" + categories + ") cannot be destroyed since the Services " + servicesListOrphanCheckServices + " in its servicesList field has a non-nullable categories field.");
            }
            if (illegalOrphanMessages != null) {
                throw new IllegalOrphanException(illegalOrphanMessages);
            }
